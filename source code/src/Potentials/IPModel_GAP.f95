...
subroutine IPModel_GAP_Calc(this, at, e, local_e, f, virial, local_virial, args_str, mpi, error)
  ...
  integer :: d, i, j, n, m, i_coordinate, i_pos0, mm

  real(dp), dimension(:,:), allocatable :: f_in
  
  real(dp) ::  norm_mi
  real(dp), dimension(3) :: pos, f_gp,mag
...
!$omp do schedule(dynamic)
     ...
     loop_over_descriptor_instances: do i = 1, size(my_descriptor_data%x)
        ...
        if(present(f) .or. present(virial) .or. present(local_virial)) then
           ...
           do n = lbound(my_descriptor_data%x(i)%ii,1), ubound(my_descriptor_data%x(i)%ii,1)  ! ii(0:n_neighbour)  lbound=0, ii(0)=i
              ...
              f_gp = matmul( gradPredict,my_descriptor_data%x(i)%grad_data(:,:,n)) * my_descriptor_data%x(i)%covariance_cutoff + &
                                  e_i * my_descriptor_data%x(i)%grad_covariance_cutoff(:,n)   !  grad_data--n  neigbour j
                  
              if( present(f) ) then
                                !  call print('=== ==========================i========================'//i) 
                                !  call print('=======j==='//j//lbound(my_descriptor_data%x(i)%ii,1)) 
                                 !   call print('=====f_gp===='//f_gp) 
                                   
               if(i==j)  then
                                          !   call    call print('=== ==========================i/=j========================'//i)
                            f_in(:,j) = f_in(:,j) - f_gp    ! 是j受到的力  double counting/2
                               !   else
                   !       f_in(:,j) = f_in(:,j) - 2*f_gp
                endif
