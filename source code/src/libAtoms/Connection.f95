...
contains
  ...
  subroutine connection_fill(this, N, Nbuffer, pos, lattice, g, origin, extent, nn_guess, store_rij, error)
    type(Connection),   intent(inout) :: this
    integer,            intent(in)    :: N    ! total number of atoms
    integer,            intent(in)    :: Nbuffer    ! Buffer size
    real(dp), optional, intent(in) :: pos(:,:), lattice(3,3), g(3,3)
    real(dp), optional, intent(in) :: origin(3), extent(3,3)
    integer, optional, intent(in) :: nn_guess
    ...
    do i=1,Nbuffer
	  ...
	  if (do_store_rij) then
	     call allocate(this%neighbour1(i)%t,4,4, 0, 0, max(do_nn_guess, 1))  !????  allocate???? ????call allcoate 
		                                                                       ! ????  allocate_table ??
	  else
  ...
  subroutine test_form_bond(this, cutoff, use_uniform_cutoff, Z, pos, lattice, i,j, shift, check_for_dup, error)
    ...
    if (d < use_cutoff) then
       call add_bond(this, pos, lattice, i, j, shift, d, error=error)   ! ?????neighbour ?????cutoff???
       PASS_ERROR(error)
    end if
   ...
   subroutine remove_bonds(this, at, bonds, error)
    ...
    subroutine calc_lat_eff(at, lat_eff, lat_eff_inv, lat_offset)
        ...
        if (any(.not. at%is_periodic)) then
            allocate(lat_pos(3,at%N))  ! N atoms every atom (x,y,z) coordinats
            lat_pos = at%g .mult. at%pos !at%pos (3:N) atom position  transfer into direct coordinates
            do i=1,3
...
!    integer  :: n1,n2
    ...
    if (present(cutoff_skin)) then
       if (cutoff_skin .fne. 0.0_dp) then
          ...
          if (.not. allocated(this%last_connect_pos) .or. &
               ...
               (size(this%last_connect_pos, 2) /= at%N) .or. &  ! size(A,2)  the number of second dimension here = N
               (at%lattice .fne. this%last_connect_lattice)) then  ! defined operator compare matrix lattice......
             ...
             do i=1, at%N
                pos_change = distance_min_image(at, i, this%last_connect_pos(:, i))  ! in atom_type.f95
    ...
    call calc_lat_eff(at, lat_eff, lat_eff_inv, lat_offset)    ! if all periodic, lat_eff = this%lattice
    call divide_cell(lat_eff, cutoff, cellsNa, cellsNb, cellsNc)   
	                                                             ! ???? lattice/cutoff
    ...
    cell_image_Na = max(1,(cell_image_Na+1)/2)  ! integer type Cell_image_Na....  always truncte decimal 
    cell_image_Nb = max(1,(cell_image_Nb+1)/2)
    cell_image_Nc = max(1,(cell_image_Nc+1)/2)  ! ????,?fit_box_in_cell ??? ??????
    ...
    if (do_fill) then
       volume_per_cell = cell_volume(at%lattice)/real(cellsNa*cellsNb*cellsNc,dp)   ! volume_per_subcell with cutoff
    ...
    do k = 1, cellsNc
	  ...
	  do i = 1, cellsNa
	     ...
	     call get_min_max_images(at%is_periodic, cellsNa, cellsNb, cellsNc, &
	        cell_image_Na, cell_image_Nb, cell_image_Nc, i, j, k, change_i, change_j, change_k, &
	        min_cell_image_Na, max_cell_image_Na, min_cell_image_Nb, max_cell_image_Nb, min_cell_image_Nc, max_cell_image_Nc)
	     change_i = .false.   ! ?????????   min max +- cell_image_Na
	     change_j = .false.
	     change_k = .false.
             atom1 = this%cell_heads(i, j, k)
             atom1_loop: do while (atom1 > 0)
		...
		do k2 = min_cell_image_Nc, max_cell_image_Nc    ! -1, 0,  +1 
		   ...
		   do j2 = min_cell_image_Nb, max_cell_image_Nb
		      ...
		      do i2 = min_cell_image_Na, max_cell_image_Na    !zhuangbi ????-1,0,1 do i2=-1,1
			 ! the stored cell we are in                 
			 if(cellsNa > 1) i3 = mod(i+i2-1+cellsNa,cellsNa)+1   ! if i2=0, i3=i, i4=0 ??????? i3=i,Na

			 ! the shift we need to get to the cell image
			 i4 = (i+i2-i3)/cellsNa      !i4=i2    ?????????????? ??cell??image cell  ??????????,????????

			 ! The cell we are currently testing atom1 against is cell(i3,j3,k3)
			 ! with shift (i4,j4,k4)
			 ! loop over it's atoms and test connectivity if atom1 < atom2

                         atom2 = this%cell_heads(i3, j3, k3)  !first entry in this subcell and loop over all atoms in this subcell by next atom in cell
                         atom2_loop: do while (atom2 > 0)

			    ! omit atom2 < atom1
			    if (atom1 > atom2) then 
                               atom2 = this%next_atom_in_cell(atom2)  ! next atom in cell(atom2 ) = cell(atom2+1)
                               cycle atom2_loop
                            endif
			    ...
			    if (.not. my_own_neighbour .and. &
				 ...
				 (i4==0 .and. j4==0 .and. k4==0) .and. &
				 (i==i3 .and. j==j3 .and. k==k3))) then
                               atom2 = this%next_atom_in_cell(atom2)  ! if they are in the same cell  next atom in cell cycle atoms2_loop
                               cycle atom2_loop
                            endif
			    call test_form_bond(this, cutoff, .true., &       ! add neighbour to neighbour1
			      at%Z, at%pos, at%lattice, atom1,atom2, &       ! loop over all atom2 including image cells
				 (/i4-map_shift(1,atom1)+map_shift(1,atom2),j4-map_shift(2,atom1)+map_shift(2,atom2),k4-map_shift(3,atom1)+map_shift(3,atom2)/), &
				 .false., error)
                            PASS_ERROR(error)
                ...
                atom1 = this%next_atom_in_cell(atom1)  ! cycle all atoms in subcell
	     end do atom1_loop ! atom1
	  end do ! i
       end do ! j
    end do ! k
  ...
  subroutine connection_calc_dists(this, at, parallel, error)
   ...
   subroutine get_min_max_images(is_periodic, cellsNa, cellsNb, cellsNc, cell_image_Na, cell_image_Nb, cell_image_Nc, i, j, k, do_i, do_j, do_k, &
	 ...
	 if (is_periodic(1)) then
	   ...
	   if (cell_image_Na < i) then  ! ???????  ???image——Na
	      min_cell_image_Na = -cell_image_Na
	   else
	      min_cell_image_Na = -i+1  ! ??? ?????????? ????????????image cell?  ??????
	   endif
  ...
  !
  !% Spatially partition the atoms into cells. The number of subcells in each dimension must already be
...
!!    ! Make sure all atomic positions are within the cell
   !! call map_into_cell(at)
    ...
    do n = 1, at%N
       ...
       ! identify the cell number of pos  i,j,k
       call cell_of_pos(this, lat_pos, i, j, k)
       !Add the atom to this cell
       this%next_atom_in_cell(n) = this%cell_heads(i, j, k)  !?ijk????? ???n?? n???subcell??????
...
!    end function cell_contents
   ...
   !% subcells such that a sphere of radius 'cutoff' with centre in    2*cutoff???????
  ...
  function connection_neighbour(this, at, i, n, distance, diff,  cosines, shift, index, max_dist, jn, error,e_i,e_j,m_j,e_ij) result(j)
    type(Connection),   intent(in)  :: this
    type(Atoms),        intent(in)  :: at
    integer,            intent(in)  :: i, n
    real(dp), optional, intent(out) :: distance
    real(dp), dimension(3), optional, intent(out) :: diff
    real(dp), optional, dimension(3), intent(out) ::    e_i, e_j, m_j
    real(dp), optional :: e_ij
    ...
    real(dp)::mydiff(3), norm_mydiff, norm_ediff
    integer ::myshift(3)
    ...
    if(present(distance)) then
     ...
     if(present(e_i) .or. present(e_j) .or. present(m_j)) then
      
        e_i=  at%spin(:,i)
        e_j=  at%spin(:,j) 
        m_j=  at%mag(:,j)
         end if
     if(present(e_ij))  then
          norm_ediff=sqrt(at%spin(1,i)*at%spin(1,i)+ at%spin(2,i)*at%spin(2,i)+at%spin(3,i)*at%spin(3,i))*&
                      sqrt(at%spin(1,j)*at%spin(1,j)+ at%spin(2,j)*at%spin(2,j)+at%spin(3,j)*at%spin(3,j))   
          e_ij=dot_product(at%spin(:,i),at%spin(:,j))/norm_ediff
      endif
  ...
  function connection_is_min_image(this, i, error) result(is_min_image)
   ...
   subroutine fit_box_in_cell(rx,ry,rz,lattice,Na,Nb,Nc)
     ...
     !in terms of the lattice vectors at eight corners, taking the maximum in the lattice directions and
     ...
     do k = -1, 1, 2
        do j = -1, 1, 2   ! ????? cutoff ????????? ??????? ?????cutoff??? ???
           do i = -1, 1, 2  ! 1 -1 1 1 1 -1 ????? ??sqrt(3)*cutoff
              lattice_coords = g .mult. (/i*rx,j*ry,k*rz/)  ! ???xyz??
