...
module  atoms_module
  ...
  public :: shuffle, n_neighbours, neighbour, centre_of_mass, atoms_copy_without_connect
  ...
  private :: atoms_calc_dists
  interface calc_dists
     module procedure atoms_calc_dists
...
contains
  ...
  subroutine atoms_initialise(this,N,lattice,&
    ...
    if (present(properties)) then
       ...
       PASS_ERROR(error)
       call add_property(this, 'spin', 0.0_dp, n_cols=3, error=error)
       PASS_ERROR(error)
       call add_property(this, 'mag', 0.0_dp, n_cols=3, error=error)
       PASS_ERROR(error)
...
!       to%ref_count = 0
  ...
  function atoms_neighbour(this, i, n, distance, diff,  cosines, shift, index, max_dist, jn, alt_connect, error, e_i,e_j,m_j,e_ij) result(j)
    type(Atoms), intent(in)         :: this
    integer ::i, j, n
    ...
    if (present(alt_connect)) then
       j = neighbour(alt_connect, this, &
            i, n, distance, diff,  cosines, shift, index, max_dist, jn, error,e_i,e_j,m_j,e_ij)  !connection.f95 connection_neighbour
       PASS_ERROR(error)
    else
       j = neighbour(this%connect, this, &
            i, n, distance, diff, cosines, shift, index, max_dist, jn, error,e_i,e_j,m_j,e_ij)
       PASS_ERROR(error)
    endif
  ...
  subroutine atoms_calc_connect(at, alt_connect, own_neighbour, store_is_min_image, skip_zero_zero_bonds, store_n_neighb, max_pos_change, did_rebuild, error)
    type(Atoms),                intent(inout)  :: at
    ...
    if (at%pot_should_do_nn) at%pot_needs_new_connect = .true.

    if (present(alt_connect)) then
       call calc_connect(alt_connect, at, &
            own_neighbour, store_is_min_image, skip_zero_zero_bonds, store_n_neighb, at%cutoff_skin, &
            max_pos_change, did_rebuild, error)
       PASS_ERROR(error)
    else
       call calc_connect(at%connect, at, &
            own_neighbour, store_is_min_image, skip_zero_zero_bonds, store_n_neighb, at%cutoff_skin, &
            max_pos_change, did_rebuild, error)
       PASS_ERROR(error)
    endif
  ...
  subroutine atoms_calc_dists(this, alt_connect, parallel, error)
    type(Atoms), intent(inout) :: this
    type(Connection), optional, intent(inout) :: alt_connect
  ...
  end subroutine atoms_calc_dists
