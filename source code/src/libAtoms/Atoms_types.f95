...
module Atoms_types_module
  ...
  type Connection
     ...
     integer, allocatable, dimension(:,:,:) :: cell_heads          !% First entry(atom no.) in cell atoms structure
  ...
  type Atoms
     ...
     integer,  pointer, dimension(:)   :: Z      => null()  !% Atomic numbers (this is not atom no. it is atomic number yuanzixushu) , dimension is actually $(N)$
	 ...
	 real(dp), pointer, dimension(:,:) :: spin   => null()  !% unit spin direction of atom i is spin(:,i)
      real(dp), pointer, dimension(:,:) :: mag   => null()  !% unit spin direction of atom i is mag(:,i)
  ...
  !% This interface calculates the distance between
  !the nearest periodic images of two points (or atoms).  ???????? 
...
contains
       ...
       if (this%N == 0) then
          ...
          case('pos')
             if (this%properties%entries(i)%type /= T_REAL_A2) &
                  call system_abort('Confused by Atoms property "pos" not of type T_REAL_A2')
             this%pos             => this%properties%entries(i)%r_a2(:,:)
          case('spin')  
                  if (this%properties%entries(i)%type /= T_REAL_A2) &
                  call system_abort('Confused by Atoms property "spin" not of type T_REAL_A2')
             this%spin             => this%properties%entries(i)%r_a2(:,:)
          case('mag')  
                  if (this%properties%entries(i)%type /= T_REAL_A2) &
                  call system_abort('Confused by Atoms property "spin" not of type T_REAL_A2')
             this%mag             => this%properties%entries(i)%r_a2(:,:)
          case('velo')
             if (this%properties%entries(i)%type /= T_REAL_A2) &
          ...
          case('pos')
           ...
           case('spin')
             if (this%properties%entries(i)%type /= T_REAL_A2) &
                  call system_abort('Confused by Atoms property "mass" not of type T_REAL_A2')
             this%spin             => this%properties%entries(i)%r_a2(:,1:this%N)
            case('mag')  
                  if (this%properties%entries(i)%type /= T_REAL_A2) &
                  call system_abort('Confused by Atoms property "spin" not of type T_REAL_A2')
             this%mag             => this%properties%entries(i)%r_a2(:,:)
  ...
  subroutine vec_map_into_cell(pos, lattice, g, shift, mapped)   ! map real position change  
    real(dp), intent(inout) :: pos(3)                    !???? ???????? 0-0.5?? ????0.5? ??????????image cell?
    real(dp), intent(in) :: lattice(3,3), g(3,3)   !????????????  ??????????image ??
   ...
   do n=1,3
      if (abs(lattice_coord(n)) > 0.5_dp) then             !??0.5??????,??????? map_real_pos_change
  ...
  function distance8_vec_vec(this,v,w,shift)
    ...
    dvw = v - w  ! array
    call map_into_cell(dvw, this%lattice, this%g, i_shift)   ! get real position change   ???????
    lattice_coord = this%g .mult. dvw  

    init_val = (/-1,-1,-1/)

    !work out which block of 8 cells we are testing  ???cell?? ???? ???????block of 8 cells
    where (lattice_coord > 0.0_dp) init_val = 0  ! where apply the rule to all element in array  the same for init_val
