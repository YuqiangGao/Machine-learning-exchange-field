...
module descriptors_module
   ...
   integer, parameter, public :: DT_SOSD            = 32
...
#include "descriptors_noncommercial_types.inc"
...
#ifdef DESCRIPTORS_NONCOMMERCIAL
   public :: sosd, soap, general_monomer, bispectrum_so4, bispectrum_so3, behler, distance_2b, &
        ...
        soap_turbo
#else
   public :: sosd, soap, bispectrum_so4, bispectrum_so3, behler, distance_2b, &
        coordination, angle_3b, co_angle_3b, co_distance_2b, cosnx, trihis, water_monomer, &
   ...
   interface initialise
      ...
      atom_real_space_initialise, power_so3_initialise, power_SO4_initialise, soap_initialise, sosd_initialise &
   ...
   interface finalise
      ...
      A2_dimer_finalise, AB_dimer_finalise, atom_real_space_finalise, power_so3_finalise, power_SO4_finalise, soap_finalise, sosd_finalise, distance_Nb_finalise,  &
   ...
   interface calc
      ...
      power_so3_calc, power_SO4_calc, soap_calc, sosd_calc, rdf_calc, as_distance_2b_calc, &
   ...
   interface cutoff
      ...
      power_so3_cutoff, power_SO4_cutoff, soap_cutoff, sosd_cutoff, alex_cutoff, distance_Nb_cutoff, rdf_cutoff, as_distance_2b_cutoff, &
   ...
   interface descriptor_sizes
      ...
      power_so3_sizes, power_SO4_sizes, soap_sizes, sosd_sizes, rdf_sizes, as_distance_2b_sizes, &
...
#include "descriptors_noncommercial.inc"
   ...
   function get_descriptor_type(args_str,error)
      ...
      logical :: is_bispectrum_so4, is_bispectrum_so3, is_behler, is_distance_2b, is_coordination, is_angle_3b, &
         ...
         is_AB_dimer, is_bond_real_space, is_atom_real_space, is_power_so3, is_power_so4, is_soap, is_sosd &  
	  ...
	  call param_register(params, 'sosd', 'false', is_sosd, help_string="Type of descriptor is sosd.")
      ...
      is_soap, is_sosd, is_AN_monomer, is_general_monomer, is_general_dimer, is_general_trimer, is_rdf, is_as_distance_2b, is_molecule_lo_d, is_alex, is_com_dimer, &
   ...
   subroutine descriptor_initialise(this,args_str,error)
	  ...
	  case(DT_SOSD)
	     call initialise(this%descriptor_sosd,args_str,error)
      case(DT_RDF)
   ...
   subroutine descriptor_finalise(this,error)
      ...
      selectcase(this%descriptor_type)
		 ...
		 case(DT_SOSD)
		    call finalise(this%descriptor_sosd,error)
         case(DT_RDF)
   ...
   subroutine RadialFunction_finalise(this,error)
      ...
      if(.not. this%initialised) return    ! return  terminate present subroutine  
      this%cutoff = 0.0_dp
      this%min_cutoff = 0.0_dp
      this%n_max = 0

      if(allocated(this%RadialTransform)) deallocate(this%RadialTransform)   ! deallocate the data in radialFunction
   ...
   subroutine soap_initialise(this,args_str,error)
      ...
      do i = 1, this%n_max
         do j = 1, this%n_max
            covariance_basis(j,i) = exp(-alpha_basis * (this%r_basis(i) - this%r_basis(j))**2)  !Gaussian radial basis function
            ...
            overlap_basis(j,i) = ( exp( -alpha_basis*(this%r_basis(i)**2+this%r_basis(j)**2) ) * &
               sqrt(2.0_dp) * alpha_basis**1.5_dp * (this%r_basis(i) + this%r_basis(j)) + &
               alpha_basis*exp(-0.5_dp * alpha_basis * (this%r_basis(i) - this%r_basis(j))**2)*sqrt(PI)*(1.0_dp + alpha_basis*(this%r_basis(i) + this%r_basis(j))**2 ) * &
               ( 1.0_dp + erf( sqrt(alpha_basis/2.0_dp) * (this%r_basis(i) + this%r_basis(j)) ) ) )  !A*A overlap
         enddo
      enddo

      !overlap_basis = overlap_basis * sqrt(pi / ( 8.0_dp * alpha_basis ) )
      overlap_basis = overlap_basis / sqrt(128.0_dp * alpha_basis**5)  ! overlap_basis*atom_sigam**2.5/8*sqrt2

      call initialise(LA_covariance_basis,covariance_basis)  ! copy covariance_basis to LA_covariance_basis%matrix
      call initialise(LA_overlap_basis,overlap_basis)
      call LA_Matrix_Factorise(LA_overlap_basis, this%cholesky_overlap_basis)   ! Cholesky decomposition
      do i = 1, this%n_max
         do j = 1, i-1 !i + 1, this%n_max
            this%cholesky_overlap_basis(j,i) = 0.0_dp  ! only keep the lower triangular matrix
         enddo
      enddo
   ...
   !!! this starts sosd, new function
   
      subroutine sosd_initialise(this,args_str,error)
      type(soap), intent(inout) :: this
      character(len=*), intent(in) :: args_str
      integer, optional, intent(out) :: error
   ...
   subroutine bispectrum_so3_calc(this,at,descriptor_out,do_descriptor,do_grad_descriptor,args_str,error)
      ...
      !   enddo
         ...
         n_i = 0
         do n = 1, n_neighbours(at,i) 
            j = neighbour(at, i, n, distance = r_ij, cosines=u_ij, diff=d_ij, shift=shift_ij)   ! atoms.f95  atoms_neighbour
...
! soap_calc 20 takes 0.0052 s
         n_i = 0
         do n = 1, n_neighbours(at,i) 
            j = neighbour(at, i, n, distance = r_ij, cosines=u_ij, diff=d_ij, shift=shift_ij) ! return the atom index of nth neighbour of atom i
            if( r_ij >= this%cutoff ) cycle
                  ...
                  if( l == 0 ) then
                     ...
                     if(arg_bess == 0.0_dp) then
                        ...
                        if(my_do_grad_descriptor) then
                           ...
                           mo_spher_bess_fi_ki_l = mo_spher_bess_fi_ki_lmm - (2*l-1)*mo_spher_bess_fi_ki_lm / arg_bess   ! get il(r_basis)  n+1  n-1  n all minus 1
                        endif
                     endif
                  endif
...
!$omp end parallel
   ...
   !!!! this part is writen by Yuqiang Gao  sosd model
   
     subroutine sosd_calc(this,at,descriptor_out,do_descriptor,do_grad_descriptor,args_str,error)
      type real_2d_array
         type(real_2d), dimension(:,:,:), allocatable :: x
      endtype real_2d_array
      ...
      real(dp), dimension(:,:), allocatable, save :: radial_fun, radial_coefficient, grad_radial_fun, grad_radial_coefficient, grad_descriptor_i
      real(dp), dimension(:), allocatable, save :: descriptor_i, radial_e_fun
      real(dp), dimension(:), allocatable :: global_fourier_so3_r_array, global_fourier_so3_i_array
      type(real_2d_array), dimension(:), allocatable :: global_grad_fourier_so3_r_array, global_grad_fourier_so3_i_array
      integer, dimension(total_elements) :: species_map
...
! soap_calc 20 takes 0.0052 s
         n_i = 0
         do n = 1, n_neighbours(at,i) 
            j = neighbour(at, i, n, distance = r_ij, cosines=u_ij, diff=d_ij, ediff=e_ij, shift=shift_ij) ! return the atom index of nth neighbour of atom i
            if( r_ij >= this%cutoff ) cycle
               ...
               do l = 0, this%l_max
                  if( l == 0 ) then
                     if(arg_bess == 0.0_dp) then
                        ...
                        mo_spher_bess_fi_ki_lm = 0.5_dp * (exp_m + exp_p) / arg_bess
                        mo_spher_bess_fi_ki_l  = 0.5_dp * (exp_m - exp_p) / arg_bess  ! function for l=0 mo sp bessel. i0(X)=sinhX/X
                        mo_spher_bess_fi_ki_l_e  = 0.5_dp * (exp(arg_bess_e) - exp(-arg_bess_e)) 
						mo_spher_bess_fi_ki_l_em  = 0.5_dp * (exp(arg_bess_e) + exp(-arg_bess_e)) 
                        if(my_do_grad_descriptor) mo_spher_bess_fi_ki_lp = mo_spher_bess_fi_ki_lm - (2*l+1)*mo_spher_bess_fi_ki_l / arg_bess
                     endif
                  else
                     if(arg_bess == 0.0_dp) then
                        ...
                        mo_spher_bess_fi_ki_lmm = mo_spher_bess_fi_ki_lm
                        mo_spher_bess_fi_ki_lm = mo_spher_bess_fi_ki_l
				        mo_spher_bess_fi_ki_emm = mo_spher_bess_fi_ki_em
                        mo_spher_bess_fi_ki_em = mo_spher_bess_fi_ki_e
                        if(my_do_grad_descriptor) then
                           mo_spher_bess_fi_ki_l = mo_spher_bess_fi_ki_lp
                           mo_spher_bess_fi_ki_lp = mo_spher_bess_fi_ki_lm - (2*l+1)*mo_spher_bess_fi_ki_l / arg_bess
                        else
                           mo_spher_bess_fi_ki_l = mo_spher_bess_fi_ki_lmm - (2*l-1)*mo_spher_bess_fi_ki_lm / arg_bess   ! get il(r_basis)
                           mo_spher_bess_fi_ki_e = mo_spher_bess_fi_ki_emm - (2*l-1)*mo_spher_bess_fi_ki_em / arg_bess 
						 endif
                     endif
                  endif
                  ...
                  !radial_fun(l,a) = exp( -this%alpha * (this%r_basis(a)**2 + r_ij**2) ) * mo_spher_bess_fi_ki_l !* this%r_basis(a)
                  radial_fun(l,a) = mo_spher_bess_fi_ki_l !* this%r_basis(a)
				  radial_e_fun(l) = mo_spher_bess_fi_ki_e*2.0_dp*exp(-2.0_dp*this%alpha_e)*2.0_dp*this%alpha_e/(1-exp(-this%alpha_e))
                  if(my_do_grad_descriptor) grad_radial_fun(l,a) = -2.0_dp * this%alpha * r_ij * mo_spher_bess_fi_ki_l + &
                     l*mo_spher_bess_fi_ki_l / r_ij + mo_spher_bess_fi_ki_lp * 2.0_dp * this%alpha * this%r_basis(a)
            ...
            sphericalycartesian_all_t = SphericalYCartesian_all(this%l_max, d_ij)
			sphericalycartesian_all_e = SphericalYCartesian_all(this%l_max, e_ij)
            if(my_do_grad_descriptor) gradsphericalycartesian_all_t = GradSphericalYCartesian_all(this%l_max, d_ij)
            do l = 0, this%l_max
               do m = -l, l
                  SphericalY_ij(l)%m(m) = SphericalYCartesian_all_t(l,m)
            ...
            do a = 1, this%n_max
               do l = 0, this%l_max
                  do m = -l, l
                     ...
                     !SPEED if(my_do_grad_descriptor) grad_fourier_so3(l,a,n_i)%mm(:,m) = grad_fourier_so3(l,a,n_i)%mm(:,m) + &
                     !SPEED    grad_radial_coefficient(l,a) * SphericalY_ij(l)%m(m) * u_ij + radial_coefficient(l,a) * grad_SphericalY_ij(l)%mm(:,m)
                     c_tmp(1) = radial_coefficient(l,a) * SphericalY_ij(l)%m(m) * SphericalE_ij(l)%m(-m)*radial_e_fun(l)
                     fourier_so3_r(l,a,i_species)%m(m) = fourier_so3_r(l,a,i_species)%m(m) + real(c_tmp(1))
                     fourier_so3_i(l,a,i_species)%m(m) = fourier_so3_i(l,a,i_species)%m(m) + aimag(c_tmp(1))
                     if(my_do_grad_descriptor) then
                        c_tmp(:) = grad_radial_coefficient(l,a) * SphericalY_ij(l)%m(m) * u_ij(:) + radial_coefficient(l,a) * grad_SphericalY_ij(l)%mm(:,m)
...
!   endsubroutine distance_Nb_calc_neighbour_loop
   ...
   function bispectrum_SO3_dimensions(this,error) result(i)
      ...
      do a = 1, this%n_max
         do l1 = 0, this%l_max
            l2 = l1               ! bispectrum  nn'lm  l1=l2  coupled L=abs(l1-l2), l1+l2
            !do l2 = 0, this%l_max   ! l1 l2  l even ?  reflection symmetry about the origin
               do l = abs(l1-l2), min(this%l_max,l1+l2)   
                  if( mod(l1,2)==1 .and. mod(l2,2)==1 .and. mod(l,2)==1 ) cycle
                  i = i + 1
               enddo
