...
module gap_fit_module
  ...
  !% everything from the command line
     ...
     config_type_sigma_string, core_param_file, gp_file, template_file, force_mask_parameter_name, mag_parameter_name
...
contains
  ...
  !% This subroutine parses the main command line options.
     ...
     character(len=STRING_LENGTH), pointer :: at_file, e0_str, local_property0_str, &
          ...
          gp_file, template_file, force_mask_parameter_name, mag_parameter_name
     ...
     mag_parameter_name => this%mag_parameter_name
     ...
     call param_register(params, 'mag_parameter_name', 'mag', mag_parameter_name, &
          help_string="Name of mag property (columns) in the input XYZ file that describes the data")
     ...
     if (len_trim(parameter_name_prefix) > 0) then
        ...
        mag_parameter_name = trim(parameter_name_prefix) // trim(mag_parameter_name)
  ...
  subroutine fit_data_from_xyz(this,error)
    ...
    logical :: has_ener, has_force, has_virial, has_stress_voigt, has_stress_3_3, has_hessian, has_local_property, &
       ...
       has_force_atom_sigma, has_force_component_sigma, has_local_property_sigma, has_force_mask, exclude_atom, has_mag
    ...
    real(dp), dimension(:,:), pointer :: f, hessian_eigenvector_i, f_hessian, force_component_sigma,mag
    ...
    n_force_component_sigma, n_hessian_sigma, n_virial_sigma, n_local_property_sigma, n_descriptors,xx
    ...
    do n_con = 1, this%n_frame
       ...
       has_local_property = assign_pointer(this%at(n_con),this%local_property_parameter_name,local_property)
       has_mag = assign_pointer(this%at(n_con),this%mag_parameter_name, mag)  ! assign_pointer  in dictionary.f95  let mag point to the key
          ...
          do i = 1, this%at(n_con)%N
             ...
             if( exclude_atom ) then
                ...
                do k = 1, 3
                   ...
                   force_loc(k,i) = gp_addFunctionDerivative(this%my_gp,-f(k,i)*mag(k,i),use_force_sigma)
                enddo
             endif
          enddo
       endif
             ...
             do i = 1, size(my_descriptor_data%x)
                do n = lbound(my_descriptor_data%x(i)%ii,1), ubound(my_descriptor_data%x(i)%ii,1)
                   ...
                   do k = 1, 3
                     ...
                     !do xx=1,size(my_descriptor_data%x(i)%grad_data,1)
                     !call print('cur: '//my_descriptor_data%x(i)%grad_data(xx,k,n)//xx//i)
                    !enddo
                      endif
                   enddo
                enddo
