...
! soap_calc 20 takes 0.0052 s
 ...
 !=======================================================  ================ = = 
         ...
         enddo
            ...
            sphericalycartesian_all_t = SphericalYCartesian_all(this%l_max, d_ik)
	    SphericalYCartesian_all_e = SphericalYCartesian_all(this%l_max, e_k)
             SphericalYCartesian_all_ei = SphericalYCartesian_all(this%l_max, e_i)
              if(my_do_grad_descriptor) GradSphericalYCartesian_all_t = GradSphericalYCartesian_all(this%l_max, e_k)
            do l = 0, this%l_max
               do m = -l, l
                  SphericalY_ik(l)%m(m) = SphericalYCartesian_all_t(l,m)
		  SphericalE_ik(l)%m(m) = SphericalYCartesian_all_e(l,m)
                  SphericalE_i(l)%m(m) = SphericalYCartesian_all_ei(l,m)
                !  spherical0=roundE_ij(l)%m(m)   
                !  call print(real(spherical0))
                !  call print(aimag(spherical0))
                        ...
                        do m1=-l1,l1
                         do m2= -l2,l2 
                           ...
                           c_tmp  = radial_coefficient_j(0,a) * SphericalY_ij(0)%m(0)*radial_coefficient_k(0,b) * SphericalY_ik(0)%m(0) *(4/mo_spher_bess_fi_ki_0)**3*sqrt(REAL(1/(4*real(l)+2)))*radial_e_fun(l)*radial_e_fun(l1)*radial_e_fun(l2)* cg_calculate(l1,m1,l2,m2,l,m1+m2)*&
                                                                                                     SphericalE_i(l)%m(m1+m2)*SphericalE_ij(l)%m(-m1)*(-1)**m1*SphericalE_ik(l)%m(-m2)*(-1)**m2
...
!   endsubroutine distance_Nb_calc_neighbour_loop
   ...
   function sosd_3b_ex_dimensions(this,error) result(i)
      ...
      i =  count * this%n_species * this%n_max*( this%n_species * this%n_max)*2     
      !i = (this%l_max+1) * this%n_max**2 * this%n_species**2 + 1
     ! if(this%diagonal_radial) then
     !    i = (this%l_max+1) * this%n_max * this%n_species * (this%n_species+1) / 2 + 1
     ! else
