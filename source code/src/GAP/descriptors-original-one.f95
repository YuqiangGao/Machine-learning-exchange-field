...
#include "descriptors_noncommercial_types.inc"
...
#ifdef DESCRIPTORS_NONCOMMERCIAL
   public :: soap, sosd, general_monomer, bispectrum_so4, bispectrum_so3, behler, distance_2b, &
...
#else
   public :: soap, sosd, bispectrum_so4, bispectrum_so3, behler, distance_2b, &
        coordination, angle_3b, co_angle_3b, co_distance_2b, cosnx, trihis, water_monomer, &
   ...
   interface initialise
      ...
      bond_real_space_initialise, transfer_initialise, com_dimer_initialise,  soap_express_initialise, sosd_ex_initialise
#else
      module procedure descriptor_initialise, RadialFunction_initialise, fourier_so4_initialise, &
      bispectrum_SO4_initialise, bispectrum_SO3_initialise, behler_initialise, distance_2b_initialise, &
      coordination_initialise, angle_3b_initialise, co_angle_3b_initialise, co_distance_2b_initialise, cosnx_initialise, trihis_initialise, &
      water_monomer_initialise, water_dimer_initialise, A2_dimer_initialise, AB_dimer_initialise, distance_Nb_initialise,  rdf_initialise, as_distance_2b_initialise, alex_initialise, &
      atom_real_space_initialise, power_so3_initialise, power_SO4_initialise, soap_initialise, soap_turbo_initialise,sosd_ex_initialise
#endif
   ...
   interface finalise
      ...
      bond_real_space_finalise, soap_express_finalise,sosd_ex_finalise
#else
      module procedure descriptor_finalise, descriptor_data_finalise, RadialFunction_finalise, fourier_so4_finalise, cplx_2d_array1_finalise, cplx_3d_array2_finalise, &
      bispectrum_SO4_finalise, bispectrum_SO3_finalise, behler_finalise, distance_2b_finalise, coordination_finalise, angle_3b_finalise, co_angle_3b_finalise, &
      co_distance_2b_finalise, cosnx_finalise, trihis_finalise, water_monomer_finalise, water_dimer_finalise, rdf_finalise, as_distance_2b_finalise,  alex_finalise, &
      A2_dimer_finalise, AB_dimer_finalise, atom_real_space_finalise, power_so3_finalise, power_SO4_finalise, soap_finalise, distance_Nb_finalise, soap_turbo_finalise,sosd_ex_finalise
#endif
   ...
   interface calc
      ...
      AN_monomer_calc,  soap_express_calc, general_monomer_calc, general_dimer_calc, general_trimer_calc,  molecule_lo_d_calc, com_dimer_calc, bond_real_space_calc,sosd_ex_calc
#else
      module procedure descriptor_calc, descriptor_calc_array, bispectrum_SO4_calc, bispectrum_SO3_calc, behler_calc, distance_2b_calc, coordination_calc, angle_3b_calc, co_angle_3b_calc, &
      co_distance_2b_calc, cosnx_calc, trihis_calc, water_monomer_calc, water_dimer_calc, A2_dimer_calc, AB_dimer_calc,  atom_real_space_calc, &
      power_so3_calc, power_SO4_calc, soap_calc, rdf_calc, as_distance_2b_calc, &
      distance_Nb_calc, alex_calc, soap_turbo_calc, sosd_ex_calc
   ...
   interface cutoff
      ...
      power_so3_cutoff, power_SO4_cutoff, soap_cutoff, alex_cutoff, distance_Nb_cutoff, rdf_cutoff, as_distance_2b_cutoff, soap_turbo_cutoff, sosd_ex_cutoff, &
   ...
   interface descriptor_sizes
      ...
      alex_sizes, distance_Nb_sizes, soap_turbo_sizes, sosd_ex_sizes,  &
...
#include "descriptors_noncommercial.inc"
   ...
   function get_descriptor_type(args_str,error)
      ...
      logical :: is_bispectrum_so4, is_bispectrum_so3, is_behler, is_distance_2b, is_coordination, is_angle_3b, &
         ...
         is_molecule_lo_d, is_alex, is_com_dimer, is_distance_Nb, is_soap_express, is_soap_turbo, is_sosd_ex
      ...
      is_distance_Nb, is_soap_express, is_soap_turbo, is_sosd_ex /) ) /= 1) then
...
! soap_calc 20 takes 0.0052 s
         ...
         do ia = 1, this%n_species*this%n_max
            ...
            i_species = rs_index(2,ia)
            do jb = 1, ia
               b = rs_index(1,jb)  ! range (1, n_max)
               j_species = rs_index(2,jb) ! indicate the species
   ...
   endsubroutine soap_calc
    ...
    subroutine sosd_ex_calc(this,at,descriptor_out,do_descriptor,do_grad_descriptor,args_str,error)
      ...
      real(dp) :: r_ij, arg_bess, arg_bess_e, mo_spher_bess_fi_ki_0, mo_spher_bess_fi_ki_l, mo_spher_bess_fi_ki_lm, mo_spher_bess_fi_ki_lmm, mo_spher_bess_fi_ki_l_e, mo_spher_bess_fi_ki_l_em, mo_spher_bess_fi_ki_l_emm, mo_spher_bess_fi_ki_lp, &
         exp_p, exp_m, f_cut, df_cut, norm_descriptor_i, radial_decay, dradial_decay, norm_radial_decay
      real(dp), dimension(3) :: u_ij, d_ij, e_i, e_j, m_i
      real(dp) :: e_ij, norm_mi
      real(dp), dimension(:,:), allocatable, save :: radial_fun, radial_coefficient, grad_radial_fun, grad_radial_coefficient, grad_descriptor_i
...
! soap_calc 20 takes 0.0052 s
         ...
         do n = 1, n_neighbours(at,i) 
            ...
            j = neighbour(at, i, n, distance = r_ij, cosines=u_ij, diff=d_ij, e_ij=e_ij, e_i=e_i, e_j=e_j,m_i=m_i, shift=shift_ij)  
     ...
     call print('8593')
	    ...
	    SphericalYCartesian_all_e = SphericalYCartesian_all(this%l_max, e_j)
		  ...
		  SphericalE_ij(l)%m(m) = SphericalYCartesian_all_e(l,m)
               ...
               enddo
            enddo
             norm_mi=sqrt(m_i(1)**2+m_i(2)**2+m_i(3)**2)
              ...
              do  l=0, this%l_max
                     ...
                     !SPEED if(my_do_grad_descriptor) grad_fourier_so3(l,a,n_i)%mm(:,m) = grad_fourier_so3(l,a,n_i)%mm(:,m) + &
                     !SPEED    grad_radial_coefficient(l,a) * SphericalY_ij(l)%m(m) * u_ij + radial_coefficient(l,a) * grad_SphericalY_ij(l)%mm(:,m)
                    c_tmp(l) = radial_coefficient(0,a) * SphericalY_ij(0)%m(0) *(4/mo_spher_bess_fi_ki_0)**2*sqrt((2*l+1)/(2*PI))*radial_e_fun(l)*Pl(l)
                     fourier_so3_r(l,a,i_species) = fourier_so3_r(l,a,i_species) + real(c_tmp(1))
                     fourier_so3_i(l,a,i_species) = fourier_so3_i(l,a,i_species) + aimag(c_tmp(1))
                      call print('this is fourier'//fourier_so3_r(l,a,i_species)) 
                     if(my_do_grad_descriptor) then
                        do m=-l,l
                        
                          c_tmp(:) = radial_coefficient(0,a) * SphericalY_ij(0)%m(0) *(4/mo_spher_bess_fi_ki_0)**2*sqrt((2*l+1)/(2*PI))*radial_e_fun(l)*SphericalE_ij(l)%m(-m) * grad_SphericalY_ij(l)%mm(:,m)/norm_mi
               ...
               enddo   ! l
            enddo   !  a
        enddo ! n   this is loop over all of the neighbour of atom i
call print('8643')
     !    if(this%global .and. my_do_grad_descriptor) then
     !       global_grad_fourier_so3_r_array(i_desc_i)%x = grad_fourier_so3_r
     !       global_grad_fourier_so3_i_array(i_desc_i)%x = grad_fourier_so3_i
                    ...
                    !    j_species = rs_index(2,jb)
                     ...
                     do a=1,this%n_max
                      do m=1,2*l+1 
                        ...
                        grad_descriptor_i(i_pow, 1:3) = grad_descriptor_i(i_pow, 1:3) + t_g_f_rr(3*(a-1)+1:3*a,m) + t_g_f_ii(3*(a-1)+1:3*a,m)
                      enddo
                        if(do_two_l_plus_one) grad_descriptor_i(i_pow, 1:3) = grad_descriptor_i(i_pow, 1:3) / sqrt(2.0_dp * l + 1.0_dp)
                     !   if( ia /= jb ) grad_descriptor_i(i_pow, 1:3) = grad_descriptor_i(i_pow, 1:3) * SQRT_TWO
                       if(a<this%n_max) i_pow = i_pow +1
...
!$omp parallel default(none) shared(this, max_n_neigh) private(i_species, a, l, n_i)
      ...
      if(allocated(fourier_so3_i)) then
       ...
       !     do a = lbound(fourier_so3_i,2), ubound(fourier_so3_i,2)
             !  do l = lbound(fourier_so3_i,1), ubound(fourier_so3_i,1)
              !    deallocate(fourier_so3_i(:,a,i_species))
             !  enddo
        !    enddo
       !  enddo
         deallocate(fourier_so3_i)
   ...
   function descriptor_n_permutations(this,error)
      ...
      selectcase(this%descriptor_type)
         case(DT_BISPECTRUM_SO4,DT_BISPECTRUM_SO3,DT_BEHLER,DT_DISTANCE_2b,DT_COORDINATION, &
            ...
            DT_ATOM_REAL_SPACE,DT_POWER_SO3,DT_POWER_SO4,DT_SOAP,DT_SOSD_EX,DT_RDF, DT_ALEX, DT_COM_DIMER, &
