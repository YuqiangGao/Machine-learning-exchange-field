...
contains
  ...
  subroutine fit_data_from_xyz(this,error)
    ...
    real(dp) :: ener, ener_core, my_cutoff, energy_sigma, force_sigma, virial_sigma, hessian_sigma, local_property_sigma, &
       grad_covariance_cutoff, use_force_sigma, norm_mi
    ...
    do n_con = 1, this%n_frame    ! each frame  supercell of 16 Fe
       ...
       if(has_force) then
          ...
          do i = 1, this%at(n_con)%N
             ...
             if( exclude_atom ) then
              ...
              !  norm_mi=sqrt(this%at(n_con)%mag(1,i)**2+this%at(n_con)%mag(2,i)**2+this%at(n_con)%mag(3,i)**2)
                do k = 1, 3
       ...
       do i_coordinate = 1, this%n_coordinate
         ...
         !  do i = 1, size(my_descriptor_data%x)
            !  if( .not. my_descriptor_data%x(i)%has_data) cycle
               ! if(n_con==1) then 
               !    if(i==1) then
               !     my_descriptor_data%x(i)%data =1.0_dp
               !     else
               !     my_descriptor_data%x(i)%data = 0.0_dp
               !     endif
               ! endif
          ! enddo
          ...
          if( has_ener ) then
             do i = 1, size(my_descriptor_data%x)
             ! if(n_con==1) call print('=====1====='//my_descriptor_data%x(i)%data)
                if( .not. my_descriptor_data%x(i)%has_data) cycle
