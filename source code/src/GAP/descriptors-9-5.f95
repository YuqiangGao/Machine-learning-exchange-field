                    ...
                    c_tmp = radial_coefficient(0,a) * SphericalY_ij(0)%m(0) *(4/mo_spher_bess_fi_ki_0)**2*sqrt((2*l+1)/(8*PI))*radial_e_fun(l)*Pl(l)
                     ...
                     if(my_do_grad_descriptor) then
                        do m=-l,l
                          ...
                          g_tmp(:) = radial_coefficient(0,a) * SphericalY_ij(0)%m(0) *(4/mo_spher_bess_fi_ki_0)**2*sqrt((2*PI)/(2*l+1))*radial_e_fun(l)*SphericalE_ij(l)%m(-m) * (-1)**m*grad_SphericalY_ij(l)%mm(:,m)/norm_mi
